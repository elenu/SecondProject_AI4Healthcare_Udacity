...
class SlicesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        volume_id, slice_id = slc
        volume = self.data[volume_id]

        image_slice = volume["image"][slice_id][None, :]  # [1, W, H]
        seg_slice = volume["seg"][slice_id][None, :]      # [1, W, H]

        sample["image"] = torch.from_numpy(image_slice).float()
        sample["seg"] = torch.from_numpy(seg_slice).long()
