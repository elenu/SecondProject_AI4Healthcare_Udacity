...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    # DICOM-viewer style corner overlay: patient name/ID/modality top-left, study time top-right
    draw.text((10, 5), f"{getattr(header, 'PatientName', 'N/A')}", fill=cyan, font=main_font)
    draw.text((10, 30), f"{header.PatientID}", fill=cyan, font=main_font)
    draw.text((10, 55), f"{header.Modality}", fill=cyan, font=main_font)
    study_time = getattr(header, "StudyTime", "")
    time_str = f"{study_time[0:2]}:{study_time[2:4]}:{study_time[4:6]}" if len(study_time) >= 6 else "N/A"
    draw.text((870, 5), time_str, fill=cyan, font=main_font)

    draw.text((10, 90), "HippoVolume.AI", fill=white, font=header_font)
    draw.multiline_text((10, 145),
                        f"Patient ID: {header.PatientID}\n"
                        f"Accession Number: {getattr(header, 'AccessionNumber', 'N/A')}",
                        fill=white, font=main_font)
    draw.multiline_text((10, 205),
                        f"Anterior: {inference['anterior']}\n"
                        f"Posterior: {inference['posterior']}\n"
                        f"Total Volume: {inference['total']}",
                        fill=white, font=main_font)
    draw.text((10, 285), f"Showing prediction at axial slices {slice_nums}", fill=white, font=main_font)

    # Overlay the predicted mask on three representative slices
    tile_size = 300
    gap = 20
    for i, slice_num in enumerate(slice_nums):
        ...
        base_img = Image.fromarray(nd_img, mode="L").convert("RGBA").resize((tile_size, tile_size))

        mask_rgba = np.zeros((*pred_slice.shape, 4), dtype=np.uint8)
        mask_rgba[pred_slice > 0] = [220, 20, 20, 170]
        mask_rgba = np.flip(mask_rgba, axis=(0, 1)).transpose(1, 0, 2)
        mask_img = Image.fromarray(mask_rgba, mode="RGBA").resize((tile_size, tile_size))

        combined = Image.alpha_composite(base_img, mask_img)
        pimg.paste(combined, box=(10 + i*(tile_size + gap), 330))
...
def get_series_for_inference(path):
    ...
    # Only the series produced by the HippoCrop tool is relevant to our algorithm
    series_for_inference = [f for f in dicoms if f.SeriesDescription == "HippoCrop"]
    if not series_for_inference:
        print("Error: no HippoCrop series found")
        return []
...
if __name__ == "__main__":
    ...
    inference_agent = UNetInferenceAgent(
        ...
        parameter_file_path=os.path.join(
            os.path.dirname(__file__), "..", "..", "section2", "results", "model.pth"))
    ...
    # Save alongside the routed study so it gets cleaned up with it after sending
    report_save_path = os.path.join(study_dir, "report.dcm")
    report_img = create_report(pred_volumes, header, volume, pred_label)
    save_report_as_dcm(header, report_img, report_save_path)

    # Push the report back to the PACS (Orthanc) archive
    os_command(f"storescu -v 127.0.0.1 4242 {report_save_path}")
