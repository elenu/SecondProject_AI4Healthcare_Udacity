...
def med_reshape(image, new_shape):
    ...
    # Compute the overlap between the original and target shapes along each
    # axis (target may be smaller in some dims, larger in others), then copy
    # the overlapping region into the top-left corner of the padded array.
    slices = tuple(slice(0, min(s, n)) for s, n in zip(image.shape, new_shape))
    reshaped_image[slices] = image[slices]
