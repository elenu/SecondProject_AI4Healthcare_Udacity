...
def LoadHippocampusData(root_dir, y_shape, z_shape):
    ...
    # (hippocampus_118.nii.gz has no label in this dataset)
    images = [f for f in listdir(image_dir) if (
        isfile(join(image_dir, f)) and f[0] != "." and
        isfile(join(label_dir, f)))]
    ...
    for f in images:
        ...
        # Skip volumes whose image and label shapes do not match
        # (hippocampus_010.nii.gz has a corrupt label of shape (36, 50, 31)
        # while its image is (512, 512, 241))
        if image.shape != label.shape:
            print(f"Skipping {f}: image shape {image.shape} != label shape {label.shape}")
            continue
        ...
        # ANSWER: The label is a segmentation mask of discrete class indices
        # (0 = background, 1 = anterior, 2 = posterior hippocampus). med_reshape
        # returns a float array, so we cast back to int to keep valid class
        # indices for the loss function and to avoid float interpolation
        # artifacts turning into invalid class values.
